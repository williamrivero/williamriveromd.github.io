...
- **Pick the right category section.** `guides/index.html` is split into
  ...
  Adding the tile to the wrong section paints the wrong colour everywhere.
  After tiling, **always re-run `generate_latest_guides.py`** so the new card
  picks up its category tint.
