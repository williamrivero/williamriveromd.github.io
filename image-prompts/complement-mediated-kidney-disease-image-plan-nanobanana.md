# Image Plan — When Complement Injures the Kidney (C3G, aHUS/CM-TMA, IC-MPGN) · Nano Banana 2 edition
...
**Paste target:** **Nano Banana 2** (Gemini 3 Pro Image) — Google Gemini app (gemini.google.com) or Google AI Studio (aistudio.google.com); also on the Gemini API / Vertex AI as `gemini-3-pro-image`.

**How Nano Banana 2 differs from GPT — read before generating:**
- **Size = aspect ratio + resolution tier, not exact pixels.** Pick the aspect ratio named in each block and the **2K** tier (choose **4K** for the hero and the large posters when offered), then export/crop to the target pixel size. Native aspect ratios: 1:1, 2:3, 3:2, 3:4, 4:3, 4:5, 5:4, 9:16, 16:9, 21:9.
- **The OG card (1.91:1) is not a native ratio** — generate it at **16:9** and crop to **1200 × 630**, keeping the title inside a centered safe area (handled in that prompt).
- **No separate "negative prompt" field.** Nano Banana reads constraints as plain sentences, so the old NEGATIVE block is now **"CONSTRAINTS"** — paste it as part of the prompt.
- **Text rendering is a strength.** Nano Banana 2 renders small labels far more legibly than GPT-image, so keep every on-image label; still name the font (Inter) so the style stays on-house.
- **Iterate conversationally.** If a figure is close, refine it with a follow-up instruction ("make the amber caution strip taller," "enlarge the LM/IF/EM headers") instead of regenerating from scratch.
- **SynthID watermark is automatic and invisible** — it does not replace our visible `© renalcarematters.com` attribution, which every figure still needs (except the wordless hero).
...
RESOLUTION / EXPORT: 2048 × 2048 — set the aspect ratio above, generate at the 2K tier (4K where offered), then export/crop to this pixel target
...
CONSTRAINTS (paste as sentences inside the prompt — Nano Banana has no separate negative field):
...
ASPECT RATIO: generate at 16:9 (native), then crop to 1.91:1
RESOLUTION / EXPORT: 1200 × 630 — generate 16:9 at the 2K tier, then crop/export to this pixel target with the title kept in a centered safe area
...
ASPECT RATIO: 16:9
RESOLUTION / EXPORT: 1792 × 1024 — set the aspect ratio above, generate at the 2K tier (4K where offered), then export/crop to this pixel target
...
RESOLUTION / EXPORT: 1024 × 1536 — set the aspect ratio above, generate at the 2K tier (4K where offered), then export/crop to this pixel target
AUDIENCE: clinicians
...
- [ ] Generate each prompt in **Nano Banana 2** (Gemini app or AI Studio): set the aspect ratio, choose the 2K tier (4K for the hero/posters), then export/crop to the target pixel size.
- [ ] Export **PNG** (Nano Banana output carries an invisible SynthID watermark — harmless; the visible © attribution still applies); also make a **WebP** twin for each (the guide's `<picture>` blocks load WebP first, PNG fallback).
