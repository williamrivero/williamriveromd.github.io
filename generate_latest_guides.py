...
def file_to_updated(index_html: str) -> set:
    """Files whose guide-tile carries the `updated` tag in guides/index.html.

    That tag is the single source of truth for the Updated filter and the
    Updated badge, so the strip reuses it rather than inventing a second rule."""
    out = set()
    tile_re = re.compile(
        r'<a\s+[^>]*?href="([^"]+\.html)"[^>]*?data-tags="([^"]*)"|'
        r'<a\s+[^>]*?data-tags="([^"]*)"[^>]*?href="([^"]+\.html)"',
    )
    for m in tile_re.finditer(index_html):
        href = m.group(1) or m.group(4)
        tags = (m.group(2) or m.group(3) or "").split()
        if href and "updated" in tags:
            out.add(href)
    return out
...
def collect(project_dir: Path):
    ...
    updated_files = file_to_updated(index_html)
    rows = []
    for path in guides_dir.glob("*.html"):
        ...
        mod = meta(text, "article:modified_time")
        ...
        try:
            ...
            continue
        # A guide earns its place by most recent activity. For guides tagged
        # `updated`, that is the revision date — but only when it genuinely
        # post-dates publication; several guides carry a modified_time that
        # predates their own publish stamp, which max() ignores rather than
        # letting it bury the card.
        is_updated = path.name in updated_files
        shown, shown_dt = pub, dt
        if is_updated and mod:
            try:
                mdt = datetime.fromisoformat(mod)
                if mdt > dt:
                    shown, shown_dt = mod, mdt
            except ValueError:
                pass
...
def build_block(rows) -> str:
    ...
    return (
        ...
        '    <div class="latest-label">Latest and updated guides</div>\n'
...
def sync_filter_counts(guides_index_text: str) -> tuple[str, int]:
    """Refresh the static <span class="count"> on every sidebar filter chip.

    The page's own JS recomputes these on load, so a stale number is invisible
    in a browser — but the served HTML is wrong until hydration, and the values
    had drifted by up to 30 as guides were added. This mirrors the runtime rule
    exactly: whole-tag match, and topic filters never count download tiles."""
    tags_re = re.compile(
        r'<a\s+[^>]*class="[^"]*\bguide-tile\b[^"]*"[^>]*>'
    )
    tile_tags = []
    for m in tags_re.finditer(guides_index_text):
        t = re.search(r'data-tags="([^"]*)"', m.group(0))
        tile_tags.append((t.group(1).lower().split() if t else []))

    def count_for(f: str) -> int:
        if f == "download":
            return sum(1 for tg in tile_tags if "download" in tg)
        return sum(1 for tg in tile_tags if f in tg and "download" not in tg)

    changed = 0

    def repl(m):
        nonlocal changed
        head, f, mid, shown, tail = m.groups()
        if f == "all":
            return m.group(0)
        real = str(count_for(f))
        if real == shown:
            return m.group(0)
        changed += 1
        return f"{head}{f}{mid}{real}{tail}"

    out = re.sub(
        r'(<button class="filter-btn"[^>]*data-filter=")([^"]+)("[^>]*>.*?<span class="count">)(\d+)(</span>)',
        repl, guides_index_text, flags=re.S,
    )
    return out, changed
...
def main():
    ...
    # Every guide tagged `updated` is guaranteed a card — that is the point of
    # the strip being "latest AND updated". The remaining slots go to the most
    # recently published guides, and the whole set is re-sorted by date so the
    # strip still reads newest-first.
    upd = [r for r in rows if r["kind"] == "updated"][: args.count]
    new = [r for r in rows if r["kind"] != "updated"]
    featured = upd + new[: max(0, args.count - len(upd))]
    featured.sort(key=lambda r: (r["dt"], r["file"]), reverse=True)

    print(f"Latest and updated — {len(featured)} card(s) "
          f"({len(upd)} updated, {len(featured) - len(upd)} new):")
    for r in featured:
        print(f"  {r['dt'].strftime('%Y-%m-%d %H:%M')}  [{r['kind']:>7}]  {r['file']}")
    ...
    new_text, n_counts = sync_filter_counts(new_text)
    if n_counts:
        print(f"guides/index.html: {n_counts} filter chip count(s) synced")

    # Write the ordered data alongside, for reference / other consumers.
    data = [
        {"file": r["file"], "title": r["title"], "published": r["published"],
         "publish_stamp": r.get("publish_stamp", r["published"]),
         "kind": r.get("kind", "new"),
