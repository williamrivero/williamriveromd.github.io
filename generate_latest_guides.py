...
def local_thumb(project_dir: Path, og_image: str, stem: str) -> str:
    ...
    A square circular-vignette hero (og:image pointed at "*-vignette-hero")
    crops awkwardly under the strip's wide, mask-faded peeking-thumb treatment —
    the purpose-built {stem}-rg-thumb.webp (a 220x160 crop already generated by
    generate_rg_thumbs.py) reads far better there, so prefer it whenever the
    resolved image would otherwise be a vignette-hero.
    """
    base = og_image.rsplit("/", 1)[-1] if og_image else f"{stem}-og.png"
    images = project_dir / "images"
    stem_no_ext = base.rsplit(".", 1)[0]
    if stem_no_ext.endswith("-vignette-hero"):
